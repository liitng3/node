...
var server=http.createServer(function(req,res){
    ...
    if(pathName=='/list'){
        ...
        var number=urlObj.query.chapterId-1;
        var Str=JSON.stringify(chapterList[number]); 
        res.end(Str);
    }
        ...
        req.on('end',function() { 
            var dataObj = querystring.parse(dataStr);
            if(dataObj.title!=undefined && dataObj.content!=undefined){
                var list={
    ...
    else if(pathName=='/addd'){
        var chapterStr=JSON.stringify(chapterList[chapterList.length-1]); 
        res.end(chapterStr);
    }
